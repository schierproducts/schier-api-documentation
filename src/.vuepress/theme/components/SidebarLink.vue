...
function renderLink (h, to, text, active, level) {
    const isGET = text.indexOf('GET') === 0;
    const isPOST = text.indexOf('POST') === 0;
    const isPUT = text.indexOf('PUT') === 0;
    const isDELETE = text.indexOf('DELETE') === 0;
  const component = {
    props: {
      to,
      activeClass: '',
  ...
  if (level > 2) {
    ...
    if (isGET || isPOST || isPUT || isDELETE) {
        ...
        return h('RouterLink', component, [
            h('span', { class: badgeClass, }, badge),
            text
        ])
    }
